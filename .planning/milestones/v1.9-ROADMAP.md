...
- Keep reminder authorization, scheduling, expiry delivery, and foreground presentation behind one shared `KeepAwakeReminderScheduling` seam.
- Tie reminder ownership to confirmed timed-session UUIDs and confirmed `.off` outcomes so notification truth stays aligned with the actual keep-awake lifecycle.
- Reuse the existing keep-awake status row for reminder-unavailable truth instead of adding a notification-specific menu section.

**Issues Resolved:**

- Timed keep-awake no longer ends silently: longer sessions schedule one pre-expiry reminder and natural expiry now produces one end reminder.
- Launch-time notification authorization is now wired through the shipped app path without blocking menu-bar startup.
- Denied or unavailable notifications stay visible inside the existing keep-awake status surface without breaking timed-session start, countdown, or expiry behavior.
...
- `NOTF-06`, `NOTF-07`, and `NOTF-08`: notification preferences, configurable lead time, and reminder troubleshooting/history remain future-scope work.

**Technical Debt Incurred:**

- The milestone audit accepts one remaining live-proof gap: Phase 24 still lacks explicit allowed-state desktop confirmation that `> 2 分钟` sessions pre-notify while `<= 2 分钟` sessions skip the pre-expiry reminder.
- `24-VALIDATION.md` and `25-VALIDATION.md` remain draft Nyquist contracts with partial status.
